...
# -*- coding: utf-8 -*-
"""
copied from https://github.com/lingjzhu/idiolect/blob/main/compare_tokenization/tokenization.py
...
def tokenize(textA, textB, seq_len=100):
    ...
    for k, a, b in tqdm(zip(range(length), textA, textB)):
        seqA = tokenizer.encode_plus(a, add_special_tokens=False, max_length=seq_len, truncation=True,
                                     return_tensors="pt")
        seqB = tokenizer.encode_plus(b, add_special_tokens=False, max_length=seq_len, truncation=True,
                                     return_tensors="pt")
...
def train(args, model, train_a, train_b, tr_attn_a, tr_attn_b, label_tr):
    model.train()
    for i, (pair_a, pair_b, lena, lenb, y) in tqdm(enumerate(
            batchify(train_a, train_b, tr_attn_a, tr_attn_b, label_tr, size=args.training_bsz, shuffling=True))):
...
def evaluate(args, test_a, test_b, te_attn_a, te_attn_b, label_te):
    ...
    targets = []
    model.eval()
    for i, (pair_a, pair_b, lena, lenb, y) in tqdm(
            enumerate(batchify(test_a, test_b, te_attn_a, te_attn_b, label_te, size=args.test_bsz))):
